...
export const selectEntityOfAccountIdsByFollower: SelectOneFn<Args, MaybeEntity> = (
  state,
  { id: follower },
...
>(
  ...
  async ({ api, id, reload }, { getState }): Promise<MaybeEntity> => {
    ...
    const accountIds = (await readyApi.query.accountFollows.accountsFollowedByAccount(
      follower,
