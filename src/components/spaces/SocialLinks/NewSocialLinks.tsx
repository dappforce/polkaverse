...
import { getLinkBrand, getLinkIcon, hasSocialMediaProfiles, LinkLabel } from './utils'
...
const resovleLabels = (links?: string[]) => {
  const labelsWithLinks: LinkLabel[] = links ? links.map(x => getLinkBrand(x) || 'Website') : []

  return links
    ? [...labelsWithLinks, ...staticLinkLabels.filter(x => !labelsWithLinks.includes(x))]
    : staticLinkLabels
}
...
const StaticSocialLinks = ({ name, collapsed, links }: NewSocialLinksProps) => (
  <Collapse defaultActiveKey={collapsed ? undefined : [name]} ghost>
    <Panel header={<h3 className='m-0'>Social links</h3>} key={name}>
      <Form.List name={name}>{staticSocialLinks(links as string[])}</Form.List>
    </Panel>
  </Collapse>
)
...
const DynamicSociaLinks = ({ name }: NewSocialLinksProps) => (
  <Form.Item label='Social links'>
    <Form.List name={name}>{dynamicSocialLinks()}</Form.List>
  </Form.Item>
)

export const NewSocialLinks = ({ isDynamic, ...props }: NewSocialLinksProps) => {
  return isDynamic ? <DynamicSociaLinks {...props} /> : <StaticSocialLinks {...props} />
}
