...
import { FaBold, FaImage, FaItalic, FaLink, FaListUl, FaQuoteLeft } from 'react-icons/fa'
...
export const CreateLinkModal = React.memo((props: InnerToolbarBtnProps & HasEditorProps) => {
  const [open, setOpen] = useState(false)
  ...
  const onClick = () => {
    ...
    if (editor) {
      runCommand(editor, e => (nonEmptyStr(value) ? e.setLink({ href: value }) : e.unsetLink()))
    }
